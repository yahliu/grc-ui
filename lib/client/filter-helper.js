...
export const combineResourceFilters = (activeFilters, storedFilters, availableGrcFilters) => {
  Object.keys(storedFilters).forEach(key=>{
    ...
    if(availableGrcFilters) {//if availableGrcFilters, return (activeFilters ∪ storedFilters) ∩ availableGrcFilters
    ...
    else {//otherwise without availableGrcFilters parameter only return (activeFilters ∪ storedFilters)
      storedFilters[key].forEach(member=>{
        activeSet.add(member)
      })
    }
