  ...
  Object.keys(GrcFilterTypes).forEach(type=>{
    ...
    case GrcFilterTypes.severity:
      if (showSeverityFilter) {
        ...
        availableSet=new Set([
          msgs.get('finding.filter.category.severity.high', locale).toUpperCase(),
          msgs.get('finding.filter.category.severity.medium', locale).toUpperCase(),
          msgs.get('finding.filter.category.severity.low', locale).toUpperCase()
        ])
      }
      break
    }
...
//get the union of activeFilters and storedFilters
export const combineResourceFilters = (activeFilters, storedFilters, displayType) => {
  ...
  //non-finding display shouldn't have severity filters
  //remove from return set for them
