...
export function createPolicyLink(item = {}, ...param){
  ...
  return <Link to={`${config.contextPath}/configuration/local/${encodeURIComponent(item.metadata.namespace)}/${encodeURIComponent(item.metadata.name)}`}>{item.metadata.name}</Link>
}
